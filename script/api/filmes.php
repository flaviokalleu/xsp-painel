...
if (isset($_POST['confirme_adicionar_filmes'])) {
    ...
    $rating_5based              = isset($_POST["rating_5based"]) ? htmlspecialchars($_POST["rating_5based"]) : '';
    $year              = isset($_POST["year"]) ? (int) $_POST["year"] : '';
    $tmdb             = isset($_POST["tmdb"]) ? (int) $_POST["tmdb"] : '';

    if (function_exists('confirme_adicionar_filmes')) {
        $confirme_adicionar_filmes = confirme_adicionar_filmes($nome, $link, $logo, $Categoria, $adulto, $director, $genre, $actors, $plot, $backdrop_path, $releasedate, $duration, $rating, $rating_5based, $year, $tmdb);
        if (!$confirme_adicionar_filmes){
            $resposta = [
                'title' => 'Erro!',
                'msg' => 'Erro na fun�ao mudar servidor.',
                'icon' => 'error'
            ];
            echo json_encode($resposta);
            exit();
