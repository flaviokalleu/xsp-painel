...
// ==========================================================
// CONSULTA — mostra clientes vinculados ao admin/revendedor logado
// e aos revendedores que ele criou (owner_id = admin_id)
// ==========================================================
...
$sql = "SELECT
        ...
        FROM conexoes AS c
        INNER JOIN clientes AS cl ON cl.usuario = c.usuario
        LEFT JOIN streams AS s ON c.canal_atual = s.id
        WHERE cl.admin_id IN (
            SELECT id FROM admin WHERE id = ?
            UNION
            SELECT id FROM admin WHERE owner_id = ?
        )
