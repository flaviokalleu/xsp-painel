...
// **********************************************
// IMPORTANTE: Certifique-se de que o arquivo 'db.php' existe
// e contém a função 'conectar_bd()'
// **********************************************
...
// ==========================================================
$ip = $_SERVER['REMOTE_ADDR']; // Obtém o IP do cliente (compatível com IPv4 e IPv6)
...
// ==========================================================
// >>> INÍCIO DA REGRA DE CONEXÃO V17.0: OTIMIZAÇÃO PARA VOD/SÉRIES IPv6 <<<
// Esta seção foi modificada para consolidar sessões do mesmo IP,
// mesmo que o User Agent mude (Live -> VOD), evitando bloqueio por limite.
// ==========================================================

$session_existe_ip_ua = false;
$id_sessao_existente = null;

// 1. Tenta encontrar uma sessão ativa que corresponda a este IP E ESTE USER_AGENT. (Melhor Match)
// Essencial para permitir troca rápida de canal (Live para Live).
...
if ($session_data) {
    $session_existe_ip_ua = true; // Match completo: IP e UA
...
if ($session_existe_ip_ua) {
    // A. Sessão Ativa (IP + UA) Encontrada: Renovação. (Troca de Canal/Conteúdo)
    ...
    // **CHAVE para permitir VOD/Séries no mesmo IP.**
    ...
    if ($session_ip_apenas) {
        ...
        // Atualiza a sessão existente com o novo User Agent.
        // Isso impede que o cliente consuma uma nova conexão do limite.
...
if ($conexao && $username && !empty($arquivo_sem_extensao)) {
    // Se o dispositivo estava em VOD e volta para LIVE, a sessão foi consolidada (UPDATE) acima.
    // Agora, garantimos que a linha atualizada (pelo IP e User Agent) reflita o conteúdo atual.
